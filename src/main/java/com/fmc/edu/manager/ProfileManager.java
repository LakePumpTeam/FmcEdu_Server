...
public class ProfileManager {
	...
	public boolean persistParentDetail(final ParentProfile pParent) {
		boolean result = true;
		if (StringUtils.isNotBlank(pParent.getAddress().getAddress())) {
			if (RepositoryUtils.idIsValid(pParent.getAddress().getId())) {
				result = getLocationService().updateAddress(pParent.getAddress());
			} else {
				result = getLocationService().createAddress(pParent.getAddress());
			}
		}
		if (result) {
			if (RepositoryUtils.idIsValid(pParent.getId())) {
				result = getParentService().updateParentProfileDetail(pParent);
			}
		}
		return result;
	}
