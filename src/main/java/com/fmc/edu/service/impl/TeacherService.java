...
public class TeacherService {
	...
	public boolean createTeacherDetail(final TeacherProfile pTeacher) {
		boolean result = getTeacherRepository().createTeacherProfile(pTeacher);
		LOG.debug(String.format("Result of process create teacher profile: %s", result));
		if (result) {
			getTeacherRepository().createTeacherDetail(pTeacher);
			LOG.debug(String.format("Result of process create teacher detail: %s", result));
		}
		return result;
	}
}
